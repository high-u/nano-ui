...
const applyProps = (element, props, ns) => {
  ...
  Array.from(element.attributes).forEach(attr => {
    ...
    if (hasKey && attr.name === 'key') return;
...
const takeExistingElement = (tag, ns, key) => {
  ...
  while (ctx.cursor < ctx.prevNodes.length) {
    ...
    if (candidate.getAttribute && candidate.getAttribute('key') != null) continue;
    if (candidate.tagName.toLowerCase() === tag.toLowerCase() && candidate.namespaceURI === ns) {
      return candidate;
    }
  }
...
const makeH = (ns) => (tag, props = {}, ...children) => {
  ...
  const pushChild = (child) => {
    ...
    while (childContext.cursor < childContext.prevNodes.length) {
      ...
      if (candidate.nodeType === Node.ELEMENT_NODE && candidate.getAttribute('key') != null) {
        continue;
      }
    }
    if (!textNode) {
